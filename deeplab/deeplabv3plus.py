...
def res_xception_block(x,channels):
	...
	x=add([x,res])
	return x
def aspp(x,input_shape,out_stride,channels=256):
	b0=Conv2D(channels,(1,1),padding="same",use_bias=False)(x)
	...
	b1=Conv2D(channels,(1,1),padding="same",use_bias=False)(b1)
	b1=BatchNormalization()(b1)
	b1=Activation("relu")(b1)
	...
	b2=Conv2D(channels,(1,1),padding="same",use_bias=False)(b2)
	b2=BatchNormalization()(b2)
	b2=Activation("relu")(b2)	
	...
	b3=Conv2D(channels,(1,1),padding="same",use_bias=False)(b3)
	b3=BatchNormalization()(b3)
	b3=Activation("relu")(b3)
	
	out_shape=int(input_shape[0]/out_stride)
	b4=AveragePooling2D(pool_size=(out_shape,out_shape))(x)
	b4=Conv2D(channels,(1,1),padding="same",use_bias=False)(b4)
	...
	return x
def deeplabv3_plus_xception(input_shape=(512,512,3),out_stride=16,num_classes=21):
	...
	return model
def deeplabv3_plus_xception_light(input_shape=(512,512,3),out_stride=16,num_classes=21):
	img_input=Input(shape=input_shape)
	x=Conv2D(16,(3,3),strides=(2,2),padding="same",use_bias=False)(img_input)
	x=BatchNormalization()(x)
	x=Activation("relu")(x)
	...
	x=xception_downsample_block(x,64,top_relu=True)

	res=Conv2D(64,(1,1),padding="same",use_bias=False)(x)
	res=BatchNormalization()(res)	
	x=Activation("relu")(x)
	x=DepthwiseConv2D((3,3),padding="same",use_bias=False)(x)
	x=BatchNormalization()(x)
